...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    """Функция возвращает траты по заданной категории за последние три месяца от переданной даты. Формат даты YYYY-MM-DD"""
    ...
    filtered = transactions[(transactions['Категория'] == category) & (transactions['Дата платежа'] >= start_date) & (transactions['Дата платежа'] <= reference_date)]

    result = filtered.groupby('Категория', as_index=False).agg({'Сумма операции': 'sum'}).rename(columns={'Сумма операции': 'Сумма трат'})
...
#
# spending_by_category(pd.read_excel('../data/operations.xlsx'), 'Супермаркеты', '2021-11-25')
